...
const ProfitAnalysisSection: React.FC = () => {
  ...
  const profitMetrics = React.useMemo(() => {
    ...
    // Calculate Cost of Goods Sold (COGS) - direct costs using cost_nature field
    const cogs = filteredTransactions
      .filter(t => t.type === 'expense' && ((t.cost_nature === 'direct') || (t.classification?.cost_nature === 'direct')))
      .reduce((sum, t) => sum + Number(t.amount), 0);
  ...
  const breakEvenAnalysis = React.useMemo(() => {
    ...
    const fixedCosts = filteredTransactions
      .filter(t => t.type === 'expense' && ((t.cost_type === 'fixed') || (t.classification?.cost_type === 'fixed')))
      .reduce((sum, t) => sum + Number(t.amount), 0);
    
    const variableCosts = filteredTransactions
      .filter(t => t.type === 'expense' && ((t.cost_type === 'variable') || (t.classification?.cost_type === 'variable')))
      .reduce((sum, t) => sum + Number(t.amount), 0);
